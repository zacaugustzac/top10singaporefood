#!/usr/bin/env node
// Generates one static HTML file per dish into dishes/
// Requires Node 18+ (native fetch). Run: node scripts/generate-static.js
...
async function api(endpoint) {
  const r = await fetch(`${SUPABASE_URL}/rest/v1/${endpoint}`, { headers: ANON_HEADERS });
  if (!r.ok) throw new Error(`${r.status} GET ${endpoint}`);
  return r.json();
}
...
function buildPage(dish, rests) {
  const pageUrl = `${BASE_URL}/dishes/${dish.slug}`;
  const title   = `Top 10 ${dish.name} in Singapore — Voted by Community`;
  const desc    = `Community-voted top 10 ${dish.name} restaurants in Singapore. Real-time rankings updated by food lovers.`;
  const footer  = `${rests.length} restaurant${rests.length !== 1 ? 's' : ''} · sorted by community votes`;
  ...
  const schema = JSON.stringify([
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: `Top 10 ${dish.name}`, item: pageUrl }
      ]
    },
    {
      '@context': 'https://schema.org', '@type': 'ItemList',
      name: `Top 10 ${dish.name} Restaurants in Singapore`,
      description: desc, url: pageUrl,
      itemListElement: rests.map((r, i) => ({
        '@type': 'ListItem', position: i + 1,
        item: {
          '@type': 'Restaurant', name: r.name, servesCuisine: dish.name,
          address: { '@type': 'PostalAddress', streetAddress: r.address || '', addressCountry: 'SG' }
        }
      }))
    }
  ], null, 2);

  const initData = JSON.stringify({ id: dish.id, name: dish.name, slug: dish.slug, restaurants: rests });
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    --gold: #B8860B; --silver: #7a7a7a; --bronze: #8B4513;
...
<header>
  <div class="header-inner">
    ...
    <div class="auth-bar" id="auth-bar">
      <button class="google-btn" onclick="signIn()">${googleIcon}<span>Sign in with Google to vote</span></button>
    </div>
  </div>
</header>
...
<script>
  ...
  function renderAuthBar() {
    ...
    const icon = '${googleIcon.replace(/'/g, "\\'")}';
    if (currentUser) {
...
async function main() {
  ...
  const dishes = await api('dishes_with_votes?select=*&order=total_votes.desc');
  console.log(`Found ${dishes.length} dishes.\n`);

  const outDir = path.join(__dirname, '..', 'dishes');
  fs.mkdirSync(outDir, { recursive: true });

  const slugs = [];
  for (const dish of dishes) {
    process.stdout.write(`  ${dish.slug}...`);
    const rests = await api(`restaurants_with_votes?category_id=eq.${dish.id}&order=total_votes.desc`);
    fs.writeFileSync(path.join(outDir, `${dish.slug}.html`), buildPage(dish, rests), 'utf8');
    slugs.push(dish.slug);
    console.log(` ${rests.length} restaurants ✓`);
  }

  // Regenerate sitemap with static URLs
  const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url><loc>${BASE_URL}/</loc><changefreq>daily</changefreq><priority>1.0</priority></url>
${slugs.map(s => `  <url><loc>${BASE_URL}/dishes/${s}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>`).join('\n')}
</urlset>`;
  fs.writeFileSync(path.join(__dirname, '..', 'sitemap.xml'), sitemap, 'utf8');

  console.log(`\nDone! ${slugs.length} pages → dishes/`);
  console.log('Re-run after adding new restaurants to refresh rankings.');
}
